...
const registerTourGuide = async (req, res) => {
    ...
    try {
        const newTourGuide = await TourGuide.create({ email, username, password,accepted});
...
const registerTourist = async (req, res) => {
    ...
    if (age < 18) {
        return res.status(400).json({ error: 'Must be 18 or older to register as a tourist' });
    }

    try {
        const newTourist = await Tourist.create({ email, username, password, nationality, dob, job ,mobile });
...
const registerAdvertiser = async (req, res) => {
    ...
    try {
        const newAdvertiser = await Advertiser.create({ email, username, password,accepted});
...
const registerSeller = async (req, res) => {
    ...
    try {
        const newSeller = await Seller.create({ email, username, password,accepted});
