  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" className="w-full p-2 bg-blue-500 text-white rounded">
          {addMode ? 'Add Activity' : 'Update Activity'}
        </button>
      </form>
    </div>
  );
...
export default function ActivityPageJohn() {
  ...
  const handleAddActivity = async (newActivity) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/advertisers/activities`, newActivity);
      setActivities((prev) => [...prev, response.data]);
      setError(null);
    } catch (error) {
      console.error('Error adding activity:', error);
  ...
  const handleEditActivity = async (activity) => {
    try {
      // Format the activity object before sending
      const formattedActivity = {
        ...activity,
        tags: Array.isArray(activity.tags) ? activity.tags : activity.tags?.split(',').map(tag => tag.trim()),
        price: parseFloat(activity.price),
        latitude: parseFloat(activity.latitude),
        longitude: parseFloat(activity.longitude)
      };
      ...
      const response = await axios.patch(
        `${API_BASE_URL}/advertisers/updateActivity/${activity._id}`, 
        formattedActivity
      );
  ...
  const handleDeleteActivity = async (id) => {
    try {
      await axios.delete(`${API_BASE_URL}/advertiser/activities/${id}`);
      setActivities((prev) => prev.filter((activity) => activity._id !== id));
      setError(null);
    } catch (error) {
      console.error('Error deleting activity:', error);
