...
import ProfileDropdown from './ProfileDropDownAdvertiser';
...
const Navbar2 = () => {
  ...
  const menuItems = [
    ...
    {
        title: 'Sales',
        link : `/Advertisers/${id}/ActivitySales`
    }
  ];
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:8000/api/advertisers/notifications/unread/${id}`);
  ...
  const handleViewAllNotifications = () => {
    ...
    navigate(`/Advertisers/${id}/notifications`);  // Use `navigate` to redirect to the "View All Notifications" page
  };

  const handleMarkAsRead = async (notificationId) => {
    try {
      await axios.patch(`http://localhost:8000/api/advertisers/MarkAsRead/${notificationId}`);
